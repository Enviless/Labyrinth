...
scene = display(title='labyrinth', x=100, y=0, width=700, height=700)

r = 0.84  #radius of the ball in centimeter
l = 32   
wt = 0.5 #wall thickness
ctw = l/2.0 #distance from center to wall
wm = materials.rough # material of wall
wc = (150/255.0, 82/255.0, 1/255.0) #color of wall
...
h14 = Hole((-ctw/8.0, -ctw/4.0), r*1.5)

holes = [h1, h2, h3, h4, h5, h6, h7, h8, h9, h10, h11, h12, h13, h14]
...
for hole in holes:
    sp -= hole.graph
board = extrusion(pos=straight, shape=sp, material = materials.wood, color = (255/255.0,165/255.0,0/255.0))

#

# pointer_x = arrow(pos=(l-0.5, 0, 1.5*r), axis=(5,0,0), shaftwidth=0.5)
# pointer_y = arrow(pos=(l, -0.5, 1.5*r), axis=(0,5,0), shaftwidth=0.5)
# pointer_z = arrow(pos=(l, 0, 1.5*r-0.5), axis=(0,0,5), shaftwidth=0.5)

wall1 = box(pos = (ctw, 0, 1.5*r), axis = (0, 1.0, 0), size = (l + wt, wt, 3.0*r), material = wm, color = wc)
wall2 = box(pos = (-ctw,0, 1.5*r), axis = (0, 1.0, 0), size = (l + wt, wt, 3.0*r), material = wm, color = wc)
wall3 = box(pos = (0, ctw, 1.5*r), axis = (1.0, 0, 0), size = (l + wt, wt, 3.0*r), material = wm, color = wc)
wall4 = box(pos = (0,-ctw, 1.5*r), axis = (1.0, 0, 0), size = (l + wt, wt, 3.0*r), material = wm, color = wc)
...
class wallinMaze:
    def __init__(self, pos1, pos2, radius_of_ball, wall_thickness):
        ...
        self.graph = box(pos = self.pos, axis = norm(self.pos1 - self.pos2), size = (mag(self.pos1-self.pos2), wall_thickness, 3.0*radius_of_ball), material = wm, color = wc)

#wv10 = wallinMaze((ctw/2.0, ctw), (ctw/2.0, 3*ctw/4.0), r, wt)

#vertical walls
wv1 = box(pos = (ctw/4.0, 3*ctw/8.0, 1.5*r), axis = (0, 1.0, 0), size = (3*ctw/4.0 + wt, wt, 3.0*r), material = wm, color = wc)
wv2 = box(pos = (ctw/4.0, -ctw/2.0, 1.5*r), axis = (0, 1.0, 0), size = (ctw/2.0 + wt, wt, 3.0*r), material = wm, color = wc)
wv3 = box(pos = (0, 3*ctw/8.0, 1.5*r), axis = (0, 1.0, 0), size = (ctw/4.0 + wt, wt, 3.0*r), material = wm, color = wc)
wv4 = box(pos = (0, -7*ctw/8.0, 1.5*r), axis = (0, 1.0, 0), size = (ctw/4.0 + wt, wt, 3.0*r), material = wm, color = wc)
wv5 = box(pos = (-ctw/4.0, -5*ctw/8.0, 1.5*r), axis = (0, 1.0, 0), size = (ctw/4.0 + wt, wt, 3.0*r), material = wm, color = wc)
wv6 = box(pos = (-ctw/2.0, -ctw/8.0, 1.5*r), axis = (0, 1.0, 0), size = (ctw/4.0 + wt, wt, 3.0*r), material = wm, color = wc)
wv7 = box(pos = (-3*ctw/4.0, ctw/4.0, 1.5*r), axis = (0, 1.0, 0), size = (ctw/2.0 + wt, wt, 3.0*r), material = wm, color = wc)
wv8 = box(pos = (-3*ctw/4.0, -7*ctw/8.0, 1.5*r), axis = (0, 1.0, 0), size = (ctw/4.0 + wt, wt, 3.0*r), material = wm, color = wc)
wv9 = box(pos = (3*ctw/4.0, -7*ctw/8.0, 1.5*r), axis = (0, 1.0, 0), size = (ctw/4.0 + wt, wt, 3.0*r), material = wm, color = wc)
wv10 = box(pos = (-ctw/4.0, 5*ctw/8.0, 1.5*r), axis = (0, 1.0, 0), size = (ctw/4.0 + wt, wt, 3.0*r), material = wm, color = wc)


#horizontal walls
wh1 = box(pos = (3*ctw/4.0, -ctw/2.0, 1.5*r), axis = (1.0, 0, 0), size = (ctw/2.0 + wt, wt, 3.0*r), material = wm, color = wc)
wh2 = box(pos = (3*ctw/8.0, -ctw/4.0, 1.5*r), axis = (1.0, 0, 0), size = (ctw/4.0 + wt, wt, 3.0*r), material = wm, color = wc)
wh3 = box(pos = (3*ctw/8.0, -3*ctw/4.0, 1.5*r), axis = (1.0, 0, 0), size = (ctw/4.0 + wt, wt, 3.0*r), material = wm, color = wc)
wh4 = box(pos = (-ctw/8.0, -ctw/2.0, 1.5*r), axis = (1.0, 0, 0), size = (ctw/4.0 + wt, wt, 3.0*r), material = wm, color = wc)
wh5 = box(pos = (-ctw/4.0, 0, 1.5*r), axis = (1.0, 0, 0), size = (ctw/2.0 + wt, wt, 3.0*r), material = wm, color = wc)
wh6 = box(pos = (-3*ctw/8.0, 3*ctw/4.0, 1.5*r), axis = (1.0, 0, 0), size = (3*ctw/4.0 + wt, wt, 3.0*r), material = wm, color = wc)
wh7 = box(pos = (-ctw/2.0, -3*ctw/4.0, 1.5*r), axis = (1.0, 0, 0), size = (ctw/2.0 + wt, wt, 3.0*r), material = wm, color = wc)
wh8 = box(pos = (-7*ctw/8.0, -ctw/4.0, 1.5*r), axis = (1.0, 0, 0), size = (ctw/4.0 + wt, wt, 3.0*r), material = wm, color = wc)
wh9 = box(pos = (-7*ctw/8.0, -ctw/2.0, 1.5*r), axis = (1.0, 0, 0), size = (ctw/4.0 + wt, wt, 3.0*r), material = wm, color = wc)
wh10 = box(pos = (-7*ctw/8.0, -ctw/2.0, 1.5*r), axis = (1.0, 0, 0), size = (ctw/4.0 + wt, wt, 3.0*r), material = wm, color = wc)
wh11 = box(pos = (ctw/2.0, 0, 1.5*r), axis = (1.0, 0, 0), size = (ctw/2.0 + wt, wt, 3.0*r), material = wm, color = wc)
wh12 = box(pos = (7*ctw/8.0, ctw/4.0, 1.5*r), axis = (1.0, 0, 0), size = (ctw/4.0 + wt, wt, 3.0*r), material = wm, color = wc)
...
# shape of star
rt_star = shapes.rectangle(width=0.2, height=0.2)
st = paths.star(pos = (-7*ctw/8.0, -7*ctw/8.0, 1.5*r), n = 5, radius = 1.5*r)
end_point = extrusion(pos = st, shape = rt_star, color = color.yellow)

won = shapes.text(text="You Won!", height = l/10.0)
won_3D_z = vector(0,0,-r)
...
while ball.pos.z >= (-5) * ball.radius:
    ...
    st = paths.star(pos = (-7*ctw/8.0, -7*ctw/8.0, 1.5*r), n = 5, radius = 1.5*r, rotate = pi/3 * t)
    end_point.pos = st
    ...
    if scene.kb.keys:
        ...
        if k in ['left', 'right', 'up', 'down']:
            ...
            if k == 'up' and alpha < 10:
                ...
                alpha += 2
                won_3D_z = rotate(won_3D_z,-deg,v)
                g = rotate(g, -deg, v)
            ...
            if k == 'left' and beta < 10:
                ...
                beta += 2
                won_3D_z = rotate(won_3D_z,deg,u)
                g = rotate(g, deg, u)
    ...
    if abs(ball.pos.y + 7*ctw/8.0) < 0.2 and abs(ball.pos.x + 7*ctw/8.0) < 0.8:
        won_3D = extrusion(pos=[-6.0*r*norm(g),-6.0*r*norm(g)+won_3D_z], shape=won, color = color.yellow)
        break
